...
export function PortfolioStartNode({
  ...
  const handlePlay = () => {
    ...
    const agentNodes = allNodes.filter(node => reachableNodes.has(node.id) && node.type !== 'memory-node' && node.type !== 'research-area-node' && node.type !== 'research-companies-node' && node.type !== 'strategy-node' && node.type !== 'trading-account-node');
